  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(!open)} className="relative w-9 h-9 flex items-center justify-center rounded-full hover:bg-[#F7F7F7] transition-colors">
        ...
        {unreadCount > 0 && <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] bg-[#F4C430] text-[#111111] text-[10px] font-black rounded-full flex items-center justify-center px-1 shadow-sm">{unreadCount > 9 ? "9+" : unreadCount}</span>}
      </button>
      {open && (
        <div className="absolute right-0 top-12 w-[320px] max-h-[400px] bg-white rounded-2xl shadow-2xl border border-border overflow-hidden z-[100]">
          <div className="flex items-center justify-between px-4 py-3 border-b bg-[#F7F7F7]">
            ...
            <div className="flex items-center gap-2">
              {unreadCount > 0 && <button onClick={() => markAllRead()} className="text-[10px] font-bold text-[#F4C430] flex items-center gap-1"><CheckCheck className="w-3 h-3" /> Read all</button>}
              <button onClick={() => setOpen(false)}><X className="w-4 h-4 text-muted-foreground" /></button>
            </div>
          </div>
          <div className="overflow-y-auto max-h-[340px]">
            {notifications.length === 0 ? (
              <div className="py-12 text-center"><Bell className="w-8 h-8 text-muted-foreground/20 mx-auto mb-2" /><p className="text-xs text-muted-foreground font-bold">No notifications</p></div>
            ) : notifications.slice(0, 20).map(n => (
              <button key={n.id} onClick={async () => { if (!n.read) await markNotificationRead(n.id); if (n.jobId) { setOpen(false); setLocation(`/customer/job/${n.jobId}`); } }} className={cn("w-full text-left px-4 py-3 flex items-start gap-3 border-b border-border/50 hover:bg-[#F7F7F7]", !n.read && "bg-[#F4C430]/5")}>
                <div className={cn("w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5", !n.read ? "bg-[#F4C430]/20" : "bg-[#F7F7F7]")}><Info className="w-3.5 h-3.5" /></div>
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const getNavItems = () => {
    if (!user) return [];
    if (user.role === "customer" || user.role === "property_manager") return [{ label: "Home", icon: Home, href: "/customer/home" }, { label: "My Jobs", icon: Briefcase, href: "/customer/jobs" }, { label: "Profile", icon: User, href: "/customer/profile" }];
    if (user.role === "provider") return [{ label: "Jobs", icon: Briefcase, href: "/provider/jobs" }, { label: "Earnings", icon: DollarSign, href: "/provider/earnings" }, { label: "Profile", icon: User, href: "/provider/profile" }];
    if (user.role === "admin") return [{ label: "Dashboard", icon: Home, href: "/admin/dashboard" }, { label: "Vetting", icon: ShieldCheck, href: "/admin/providers" }, { label: "Jobs", icon: Briefcase, href: "/admin/jobs" }];
    return [];
  };
      ...
      {!isAuthPage && location !== "/" && (
        <header className="sticky top-0 z-50 w-full border-b bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60">
          <div className="container flex h-14 items-center justify-between px-4">
            <Link href="/"><div className="flex items-center gap-3 cursor-pointer"><img src="/menda-logo.jpg" alt="Menda" className="h-10 w-auto object-contain" /><span className="font-heading font-extrabold text-xl tracking-tighter uppercase text-[#111111] leading-none">MENDA</span></div></Link>
            <div className="flex items-center gap-3">
              {user ? (
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={handleLogout} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-red-50 text-muted-foreground hover:text-red-500 transition-colors"><LogOut className="w-4 h-4" /></button>
                </div>
              ) : !location.startsWith("/auth") && (
                <Dialog open={showRoleModal} onOpenChange={setShowRoleModal}>
                  <DialogTrigger asChild>
                    <button className="text-xs font-black uppercase tracking-[0.2em] text-[#111111] hover:text-[#F4C430] transition-all flex items-center gap-2"><LogIn className="w-3.5 h-3.5" /> Sign In</button>
                  </DialogTrigger>
                  <DialogContent className="sm:max-w-md border-none rounded-3xl shadow-2xl p-0 overflow-hidden bg-white">
                    <DialogHeader className="bg-[#111111] p-8 text-white">
                      ...
                      <DialogDescription className="sr-only">Choose which kind of Menda account you want to sign in with.</DialogDescription>
                    </DialogHeader>
                        ...
                        <Link key={role.id} href={`/auth/${role.id}`} onClick={() => setShowRoleModal(false)}>
                          <div className="flex items-center justify-between p-4 bg-[#F7F7F7] rounded-2xl hover:bg-[#F4C430] group transition-all cursor-pointer">
                            <div className="flex items-center gap-4">
                              <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center shadow-sm group-hover:bg-[#111111] group-hover:text-white transition-colors"><role.icon className="w-5 h-5" /></div>
                              <div><p className="font-black text-sm text-[#111111]">{role.label}</p><p className="text-[10px] font-bold text-muted-foreground group-hover:text-[#111111]/60">{role.description}</p></div>
                            </div>
